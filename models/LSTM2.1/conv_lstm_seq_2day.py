...
tf.app.flags.DEFINE_string('train_dir', './train_store_conv_lstm_seq_2day',
                           """dir to store trained net""")
tf.app.flags.DEFINE_integer('seq_length', 12,
                            """size of hidden layer""")
tf.app.flags.DEFINE_integer('seq_start', 7,
...
def load_data():
    # load data
    dat = np.load('../batch_data/batch_timestamp_3d.npy')
    return dat
...
def train():
    ...
    with tf.Graph().as_default():
        ...
        for step in xrange(FLAGS.max_step):  # 1 step = 1 batches training
            # dat = np.random.rand(36,12,100,100,5)
            # dat = generate_bouncing_ball_sample(FLAGS.batch_size, FLAGS.seq_length, 100, FLAGS.num_balls)

            # # batch_size * timestamp
            # index = random.randint(1, 104)
            # batch_data = np.load('./batch_data/batch_%d.npy'%index)  # [batch_size * timestamp, 100, 100, 5]

            # seq
            a = np.random.randint(0, 732 - 1 - 12)
            slices = dat[a]  # (12, 100, 100, 5)
            for i in range(1, FLAGS.batch_size):
                slices = np.vstack((slices, dat[a+i]))
            # (84, 100, 100, 5)
            batch_data = slices.reshape(FLAGS.batch_size, FLAGS.seq_length, 100, 100, 5)
