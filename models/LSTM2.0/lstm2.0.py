...
tf.app.flags.DEFINE_string('train_dir', './train_store_lstm2.0',
...
def network(inputs, hidden, lstm=True):
    ...
    if lstm:
        ...
        with tf.variable_scope('conv_lstm', initializer=tf.random_uniform_initializer(-.01, 0.1)):
            # 25 * 4 = 100
            cell = BasicLSTMCell.BasicLSTMCell([25, 25], [25, 25], 4)
            if hidden is None:
...
def train():
    ...
    with tf.Graph().as_default():
        ...
        # train_log
        if tf.gfile.Exists(FLAGS.train_dir + '/train2.0_log.log'):
            tf.gfile.Remove(FLAGS.train_dir + '/train2.0_log.log')
        train_log = open(FLAGS.train_dir + '/train2.0_log.log', 'w')

        # test_log
        if tf.gfile.Exists(FLAGS.train_dir + '/test2.0_log.log'):
            tf.gfile.Remove(FLAGS.train_dir + '/test2.0_log.log')
        test_log = open(FLAGS.train_dir + '/test2.0_log.log', 'w')
        ...
        for step in range(FLAGS.max_step + 1):  # 1 step = 1 batches training
            ...
            if step % 10000 == 0:
                ...
                save_figs_path = './fig_store_lstm2.0/Fig_step' + str(step)
                ...
                for feaID in range(5):
                    for gridID in range(0, 9999, 250):
                        ...
                        plt.xlim([0, 2016 + 144])
                        fig = plt.gcf()
                        fig_save_path = save_figs_path + '/lstm_fig_%s_grid%s' % (
