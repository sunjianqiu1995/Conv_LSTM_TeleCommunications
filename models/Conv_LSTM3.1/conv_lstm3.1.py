...
def train():
    ...
    with tf.Graph().as_default():
        ...
        # test_log
        if tf.gfile.Exists(FLAGS.train_dir+'/test_log.log'):
            tf.gfile.Remove(FLAGS.train_dir+'/test_log.log')
        test_log = open(FLAGS.train_dir+'/test_log.log','w')
        ...
        for step in xrange(FLAGS.max_step + 1):  # 1 step = 1 batches training
            ...
            if step % 10000 == 0:
                ...
                test_log.write(str(f1) + str(',') + str(f2) +str(',') + str(f3) +str(',') + str(f4) +str(',') + str(f5) + '\n')
                # var
                ...
                test_log.write(
                    str(v1) + str(',') + str(v2) + str(',') + str(v3) + str(',') + str(v4) + str(',') + str(v5) + '\n')
