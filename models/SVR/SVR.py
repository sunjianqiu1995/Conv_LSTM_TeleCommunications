...
# x axis
X = np.arange(0, 2016, 1)  # [2016,1]
X = X.reshape(2016, 1)

X_future = np.arange(0,2016+144,1).reshape(2016+144,1) #[2016+144,1]
...
def train_save_eval_model(timeseries_fea,feaID,gridID,y_true,save_fig_path,save_model_path):
    ...
    y = timeseries_fea[gridID][-2016:] #[2016]
    # #############################################################################
    ...
    plt.legend(loc='best')
    plt.title('SVR MSE: %.4f ACC: %.4f' % (MSE, acc))
    plt.xlim([0, 2016 + 144])
...
for feaID in range(len(timeseriesname)):
    filename = open('./log_test_svr_%s.log'%timeseriesname[feaID],'w')
    MSE_arr = []
    acc_arr = []
    for gridID in range(0,len(Sin),250):
        y_true = data_test[:, gridID, feaID]
        save_fig_path = './svr_predict_figure_save/svr_predict_figure_save_%s/SVR_%s_grid%d.png' % (timeseriesname[feaID], timeseriesname[feaID], gridID)
        save_model_path = './svr_train_model_store/svr_train_model_store_%s/SVR_model_%s_grid%d.pkl' % (timeseriesname[feaID], timeseriesname[feaID], gridID)
